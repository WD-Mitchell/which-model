...
| Cursor model variants | normalize and merge variants (-fast, -thinking, -max, reasoning) into a single ModelEntry; -max is context-window, not reasoning effort | Cursor exposes variants as distinct IDs; without normalization same-model variants duplicate cards, inflate route counts, and misclassify -max as reasoning (Issue #145) |
