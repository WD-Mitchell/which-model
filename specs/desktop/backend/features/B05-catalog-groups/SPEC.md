...
14. **Models list.** `Models(ctx)` returns the union of scores-CSV identities, route identities, and models advertised by enabled providers, including cached live inventories and Codex's local model catalog. Rows are deduplicated by cleaned display name and sorted name ascending. A model does not need benchmark data to be listed: unscored rows have nil intelligence/cost/speed, never fabricated zeroes. Scored rows retain the highest scored effort's values. Reasoning levels and provider ids are unioned; provider listings respect provider enablement, while existing routes continue to contribute reachability by presence. `gui.only_enabled_providers` retains its provider-count filter. The maker comes from recognized model families (GLM → Z.AI, Kimi → Moonshot AI); an unrecognized family is `Other`, never the model's first word. Reads do not fetch or emit.

**Correction — new-model visibility (2026-09-05 user decision).** This supersedes the scores-only model-list contract and its omission of route-only identities. Benchmark coverage governs scores and ranking, not whether a known model can be inspected.

15. **Model card.** `Model(ctx, name)` returns the `CatalogModelDetail` for a display name or model ID: matches identities (exact, case-insensitive, or via model ID / route mapping), setting `InCatalog: true` only when a scores row exists, or synthesizes identity from provider listings/routes/models.dev for newly released models without catalog scores yet with `InCatalog: false` (intelligence/cost/speed nil). `Providers` carries one row per ENABLED provider serving the model — unlike §2.14, whose source (a) also counts routes of disabled providers, the card filters every row on `providers.<id>.enabled` — with route keys and models.dev pricing. Aggregation is TWO-PASS because provider-native ids are provider-scoped: pass 1 admits entries matching by display name and records the providers proven to serve the model; pass 2 admits id-only matches SOLELY from those providers, so two unrelated models sharing a generic id such as `default` never cross-join. Scoping by provider rather than by "is this name a different catalog identity" is required because two UNSCORED models sharing an id have no catalog identity to arbitrate between them. Each row's `Reasoning` lists every effort level the provider EXPOSES for that model, including levels currently switched off under `[routes.disabled]`: the card is a navigation surface into per-combo benchmarks (U15 §2.6) and the Providers page owns the per-level toggles, so filtering disabled levels here would hide combos the user can still inspect and re-enable. An unknown model absent from scores, routes, and provider catalogues returns `not_found`.
