...
14. **Models list.** `Models(ctx)` returns one `CatalogModel` per distinct scores-CSV display name (`ScoreRow.Model`), sorted name ascending. Each row aggregates every reasoning level that name reports (collapsed, unique, effort-ladder order). Intelligence/cost/speed are the top effort row's tier-1 scores (nil when that axis is blank). `Providers`, `ProviderCount`, `ModelID`, and the reasoning set are the UNION of two sources, deduplicated: **(a) routes** — every `route` whose `route.Model` equals the name contributes `route.Provider`, `route.ModelID`, and `route.Reasoning` by PRESENCE, with no enablement check, because the route table records reachability; and **(b) enabled provider listings** — for each provider with `providers.<id>.enabled == true`, every `ProviderModel` whose cleaned name matches contributes its provider id, `ModelID`, and exposed levels, INCLUDING models.dev catalogue entries that carry no route yet, so a freshly enabled provider is reflected before `routes refresh` runs. A disabled provider therefore still counts through (a) if it has routes, and contributes nothing through (b). `ModelID` is the lexicographically first id in that union, or `""`. Enablement clipping of the list itself is the separate `gui.only_enabled_providers` lever, which filters to models with `ProviderCount > 0`. Route-only identities that are not in the scores CSV are omitted. Reads never emit.
