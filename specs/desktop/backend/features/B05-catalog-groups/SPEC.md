...
14. **Models list.** `Models(ctx)` returns one `CatalogModel` per distinct scores-CSV display name (`ScoreRow.Model`), sorted name ascending. Each row aggregates every reasoning level that name reports (collapsed, unique, effort-ladder order). Intelligence/cost/speed are the top effort row's tier-1 scores (nil when that axis is blank). `ModelID` is the lexicographically first `route.ModelID` whose `route.Model` equals the name, or `""`. `ProviderCount` is the number of distinct `route.Provider` values that carry a route for that name — route PRESENCE, not provider enablement: a routed provider counts even while `providers.<id>.enabled` is false, because the route table records reachability and the list is the full scores catalog (U15 §2.2), not an availability view. Enablement clipping is the separate `gui.only_enabled_providers` lever, which filters the list to models with `ProviderCount > 0`. Route-only identities that are not in the scores CSV are omitted. Reads never emit.

15. **Model card.** `Model(ctx, name)` returns the `CatalogModelDetail` for a display name or model ID: matches catalog identities (exact, case-insensitive, or via model ID / route mapping) with `InCatalog: true`, or synthesizes identity from provider listings/routes/models.dev for newly released models without catalog scores yet with `InCatalog: false` (intelligence/cost/speed nil). `Providers` carries one row per enabled provider serving the model — provider-level enablement (`providers.<id>.enabled`) IS filtered here, unlike §2.14's route-presence count — with route keys and models.dev pricing. Each row's `Reasoning` lists every effort level the provider EXPOSES for that model, including levels currently switched off under `[routes.disabled]`: the card is a navigation surface into per-combo benchmarks (U15 §2.6) and the Providers page owns the per-level toggles, so filtering disabled levels here would hide combos the user can still inspect and re-enable. An unknown model absent from scores, routes, and provider catalogues returns `not_found`.
