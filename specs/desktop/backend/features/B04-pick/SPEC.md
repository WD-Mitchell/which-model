...
4. **Availability set (B00 CONTRACTS §6.3).** `available []pick.Identity` is built from the routes table (`routing.LoadTable`, cached by B02): one canonical score identity per enabled, non-disabled route. The service resolves `route.Model` to an exact cleaned score name first, then to a unique `identity.ModelNameKey` match; an ambiguous fallback is not treated as available. It applies `identity.CollapseReasoning`, so a route labelled `default` matches the score identity `high`. Route enablement and `[routes.disabled]` still use the provider-native route tuple. Duplicates (same catalog identity via several providers) are deduplicated.
...
7. **Provider and route key per candidate.** Each candidate's `Provider` is the highest-priority (lowest `priority`, ties by id ascending — B00 §6.1) ENABLED provider whose route resolves to the same canonical score identity (exact cleaned name, then unique normalized-name fallback; reasoning uses `identity.CollapseReasoning`) and is not disabled for its provider-native route tuple under `[routes.disabled]` — the mockup's `results()` first-match rule. `ModelID` is that route's provider-native `routing.Route.ModelID`; `RouteKey = FormatRouteKey(Provider, ModelID, route.Reasoning)` so the key identifies the actual enabled route tuple. A candidate that survived §2.4 always resolves a provider by construction.
