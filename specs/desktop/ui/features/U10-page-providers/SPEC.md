...
7. **Per-model blocks.** One block per `ProviderModel`: a 168px column with model name, model id, and a ghost button whose label is `Disable all` when ANY level of that model is enabled, else `Enable all`; beside it, a max-width 420px column of level rows — `Toggle` + label `reasoning {level.reasoning}` + a neutral `default` tag when `level.default` is true (the model's top level). A model with null/empty `levels` is available from the provider but unrouted (no benchmark row): same block, NO button, and the levels column shows the muted mono marker `no routes` (B06 SPEC §2.8).
