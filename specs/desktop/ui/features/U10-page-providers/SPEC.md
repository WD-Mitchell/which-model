...
| List control state owner | U10-owned module store (`pages/Providers/listState.ts`), session lifetime | The list unmounts under any detail (U07 renders list XOR detail); only module-level state survives the round-trip. Mirrors U06's `lib/overrides.ts` ownership pattern |
