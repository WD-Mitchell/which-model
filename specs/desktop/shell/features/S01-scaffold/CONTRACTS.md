...
- **Dependency pins (§3):** the manifests are authoritative for the resolved ranges; §3 fixes the shape and the dependency set, not the numbers, and Dependabot keeps the ranges current (grouped so lock-step families such as react/react-dom move together). At implementation (2026-08-18) the ranges were react/react-dom ^19.2.8, @tanstack/react-query ^5.101.4, zustand ^5.0.15, @fontsource/inter ^5.3.0, vite ^7.1.3, vitest ^3.2.4, typescript ^5.9.3, @testing-library/react ^16.3.2, jsdom ^30.0.1 and `@vitejs/plugin-react` ^5.1.2 (v6 required Vite 8, which has since landed). `@types/react` + `@types/react-dom` were added to devDependencies — required for `tsc -b` under `strict` (not listed in §3 but necessary for the React 19 types). Corrected 2026-09-15: this note previously read as a pin list and had drifted from the manifests; a later dependency bump does not amend it.
