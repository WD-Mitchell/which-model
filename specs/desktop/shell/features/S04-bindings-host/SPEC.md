...
3. **Promise translation.** Go methods return `(DTO, error)` or `error`. Wails maps these to `Promise<DTO>` / `Promise<void>`: a nil error resolves with the JSON-serialised DTO (snake_case keys per D00 §2); a non-nil error rejects. Engine errors are `service.ErrorDTO` values that JSON-serialise to the `ErrorDTO` shape `{code, message}` (D00 §2/§4); wrappers normalize them through the canonical mapper so the rejection payload is ErrorDTO-shaped.
