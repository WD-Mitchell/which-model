...
5. **Publish modes** (annex-b §8.4; `mode` selects per invocation, not per branch):
   - `pull-request`: `gh pr create --base "${{ matrix.branch }}" --title "<pr_title>" --body "Automated catalog refresh." --label <l1> --label <l2>…` (one `--label` per `pr_labels` entry) with `env: GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}`; the fixed body makes invocation non-interactive. When `auto_merge` is true, immediately `gh pr merge --auto --<merge_method>` (same env). `gh pr merge --auto` defers to GitHub's own auto-merge machinery — branch protection is respected.
