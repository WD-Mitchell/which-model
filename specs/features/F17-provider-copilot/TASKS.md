...
2. Implement `Fetch(ctx, cred, client)` (replace placeholder), the port of `checkCopilotUsage` minus the interactive `--login` leg:
   ...
   - Success → `Snapshot{Provider:"copilot", Windows: windows, UsageKnown: slices.ContainsFunc(windows, func(w usage.Window) bool { return w.UsageKnown && !w.Synthetic }), Account: login, FetchedAt: now UTC, Source: usage.SourceOAuth, Confidence:"live"}`.
