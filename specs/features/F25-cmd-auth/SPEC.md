...
8. **Login: supported flows.** Only providers whose sole interactive flow is a device-code flow (Copilot) are supported: F25 calls F12's device-flow `Start`, prints exactly two validated fields to stdout as the primary output — `Open <verification_uri> and enter code <code>.` — writes progress (`waiting for confirmation...`) to stderr, then calls `Poll` and persists the validated token through `ManagedStore.Save`. Exit 0 means polling completed and storage succeeded; poll/save failure is `runtime` exit 1. The device code and token never appear anywhere except the validated user code in the prompt, and save errors are redacted with the token before rendering. (Source: annex-d §2.2; research spec §2.3; Decisions D-4.)
...
10. **Logout: confirmation and deletion.** `auth logout <provider>` removes credential material `which-model` itself wrote (F12 `ManagedStore.Remove`); provider-native stores (`~/.claude/credentials.json`, `~/.codex/auth.json`, keychain items the provider's own tools created, `git config`) are never touched. Removal attempts both the managed keychain item and fallback file even if the credential cannot currently resolve (revoked/invalid credentials must still be removable) and regardless of the current `use_keychain` setting. Confirmation: with `--yes`, no prompt; on a TTY without `--yes`, prompt `Remove which-model's cached credential for <provider>? [y/N]` (anything but `y`/`yes` → print `aborted` to stderr, exit 0, nothing removed); non-TTY without `--yes` → exit 2 `[arguments] refusing unattended logout without --yes`. Nothing removable → exit 0 with the message in §3 (decision D-6). (Source: research spec core.mjs:249-263; Decisions D-5/D-6.)
...
12. **Exit codes.** `auth status`: 0 when every queried provider is `ok`; 5 when at least one queried provider is `missing` or `expired` (no usable credential); 2 for argument errors (unknown provider, bad flags) and usage-disabled refusal. `auth login`: 0 only after successful device-flow completion and storage; 1 for poll/storage runtime failure; 2 for unattended refusal, unsupported provider, unknown provider, invalid flags. `auth logout`: 0 on success/abort/nothing-to-remove; 2 for unknown provider, missing required args, unattended refusal; 1 for removal runtime error. (Source: annex-d §2.2; Decisions D-3/D-6/D-9.)
...
| D-11 Managed login storage | `[auth].use_keychain = true` by default; file fallback when unavailable/disabled | Matches the F12 managed-store contract while keeping login functional on hosts without a usable keychain |
