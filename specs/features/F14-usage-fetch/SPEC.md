...
4. **Per-provider pipeline (online).** For an enabled, known provider: (a) **cache read** unless `Refresh` or `CacheTTL == 0` — TTL = `cache.EffectiveTTL(desc.CacheTTL, opts.MaxAge)` (annex-d `--max-age`); fresh → serve (step 6); miss/stale/corrupt → continue; (b) **credentials**: `credential.ResolveChain(ctx, desc.Auth, client)`; `ErrNotFound` → `Snapshot{Failure: {login_required, "no credential found for provider <id>"}}` (no fetch, no write); hard resolver error → its Failure (message scrubbed, step 9); (c) **fetch**: `desc.Fetch(ctx, cred, client)` under a per-provider context deadline (step 5); error → failure snapshot (step 9), no cache write; success → (d) **cache write** (failure → warning, never error — annex-a §6: cache is an optimization); (e) stamp `Source = SourceFor(cred, desc.Kind)`, `Confidence = ""`, `Stale = false` (global CONTRACTS §1.5; decision D3).
...
12. **Transport.** One shared client per `FetchAll` call: `httpkit.NewClient()` with DEFAULTS (F04: `DefaultTimeout 10s`, `DefaultMaxResponseBytes 262144`, redirect rejection, retries 1×500ms on 5xx/network) — no `WithTimeout`, because per-provider deadlines come from step 5's contexts and a client-level 10s would truncate slow-but-valid providers (e.g. codex 15s; decision D4). Body bounds come from httpkit's default. Auth headers are the provider's job (F04 contract: headers never set inside httpkit).
...
| D4 | Client timeouts | shared `httpkit.NewClient()` defaults + per-provider ctx deadlines | A client-level 10s (F04 default) truncates valid 15s providers (codex annex-a §6); per-provider `WithTimeout` gives each its own budget and siblings stay independent. Recorded for F08 parity (F08's AA v2 uses `WithTimeout(20s)`). |
