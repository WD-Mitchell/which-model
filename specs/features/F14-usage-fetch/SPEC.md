...
## Review corrections (#180, #181, #184)

The CodexBar backend follows the same online cache-first pipeline: use `cache.EffectiveTTL(15*time.Minute, opts.MaxAge)` for reads, bypass online reads only with `Refresh`, and return fresh cache entries before credential/keychain/subprocess work. A cache hit never writes, sets cache/cached provenance and `Stale=false`, and applies identity redaction only to the returned copy. Missing, corrupt, stale, failed, or ineligible cache entries proceed to live fetch; failures are never written. B08's normal 15-minute and force 60-second ages therefore work without unconditional refresh.
