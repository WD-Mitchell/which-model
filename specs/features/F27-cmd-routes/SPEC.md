...
5. **refresh.** `routes refresh [--auto <fuzzy-name>]`: calls F18's production pipeline (`routing.ProduceRoutes(cfg)`) and writes the result to the routes file. User-declared routes are preserved across refresh (production output never removes them; where production and user routes collide on (provider, model-id), the user route wins — F18's contract for merging; F27 persists exactly what ProduceRoutes returns). If F18 reports an ambiguous model, F27 still persists the successful routes returned with the error, then returns a runtime error; the ambiguous model itself is omitted. Idempotent: refresh when nothing changed writes the same file bytes (compare-and-skip); an F18 ambiguity still returns its error even when no write is needed. `--auto <fuzzy-name>`: after production, also add a user_declared route whose `model` fuzzy-matches the given name against available score rows (`--auto claude-sonnet` picks the closest row by substring/prefix; zero or ambiguous matches → exit 2 with `[arguments] no score row matching "x"`). (Source: annex-d §2.6; Decisions D-5.)
...
7. **verify.** `routes verify [--json]` audits the routes file against the current scores CSV (`catalog.scores_csv_path`, read via F06 `csvstore.ReadScores`):
   - Route names resolve with F07's shared `identity.ResolveModelName`: exact cleaned name first, then a unique normalized alphanumeric name. An ambiguous normalized fallback does not resolve. Stale route = its resolved `model` plus reasoning no longer identifies a score row → printed on stdout, one per line: `stale route <provider>:<model-id> (<model>/<reasoning>)`; exit 1. Zero stale routes → exit 0.
   - Unrouted score rows = score rows with no route after the same name and reasoning canonicalization → stderr warnings `warning: score row <model>/<reasoning> has no route; it cannot be picked` (no exit effect).
...
8. **Exit codes.** `add`/`remove`: 0 success; 2 argument/validation errors; 1 IO error (unreadable/writable routes file). `list`: 0 always except 2 (unknown --provider, bad flags) and 1 (IO). `refresh`: 0 success; 1 IO error or F18 routing ambiguity (the successful partial table is already persisted); 2 bad `--auto` match or unknown flags. `verify`: 0 clean; 1 stale routes present or IO error; 2 bad flags. (Source: annex-d §2.6.)
...
| `refresh` F18 ambiguity | 1 | `[runtime] <F18 ambiguity message>` after successful routes are persisted |
