...
`which-model pick` scores routed candidates, applies usage-aware band gating, and selects one model with an F20 strategy. It emits text or structured JSON and records an audit trail consumed by `explain`. Exit codes distinguish authentication, usage gating, availability, arguments, and runtime failures.
...
3. `--strategy <name>` selects the strategy. Resolution precedence is explicit `--strategy`, then non-empty `[strategy].default`, then the dynamic fallback (`closest-to-reset` if usage detection is enabled, `priority` otherwise). The full F20 `strategy.Config` is decoded once so all four canonical sibling keys and their environment overrides are accepted. Valid names are `priority`, `round-robin`, `least-used`, `most-used`, and `closest-to-reset`; unknown or removed names exit 2.
...
6. The pipeline runs in this order, each stage feeding the next (master plan README §4 routing join; Decisions D-6):
   ...
   g. **Strategy**: apply the F20 strategy. `least-used`, `most-used`, and `closest-to-reset` require usage data and exit 2 when usage is disabled; no strategy silently falls back.
7. **Result**: if at least one candidate survives, emit the ordered strategy result with the selected candidate first.
...
8. **JSON output** (`--json`, and ALWAYS when stdout is not a TTY — annex-c §4.6 agent mode; Decision D-8): `PickResult` per annex-c §4.2 + §4.6 VERBATIM:
   ...
   ```
   Field rules: `schema_version` is `"2.0"`; `usage_disabled_reason` is null when enabled and a reason string when disabled; `route` is the full Route object; `band` and `band_weight` are omitted when usage is disabled; `normalizer` and `aggregator` are the F10/Global names; `excluded_candidates` use the stable reason-code enum.
...
10. **History log**: every run appends one line to `<state_dir>/pick/history.jsonl`: `{"ulid": ..., "ts": ..., "profile": ..., "strategy": ..., "candidate_id": ..., "final_score": ..., "excluded_count": n, "evidence": {...}}`. The file is created lazily; write failures warn but do not fail selection.
...
12. **Determinism**: with the same config, catalog, usage snapshot, and persisted round-robin cursor, strategy output is reproducible.
...
15. **Degraded mode**: usage fetching and bands are skipped; `priority` and `round-robin` remain available. `least-used`, `most-used`, and `closest-to-reset` exit 2 with `usage_disabled` rather than falling back.
...
| D-3 | Strategy names come from F20; precedence is explicit `--strategy` > configured `strategy.default` > `closest-to-reset` with usage enabled or `priority` otherwise | One shared F20 config schema prevents strict-decoder split-brain; explicit user intent wins and the usage toggle supplies a viable fallback only when config is unset |
...
| D-19 | Strategy names use F20's canonical hyphenated strings | One spelling avoids CLI/registry drift |
