...
3. Implement `Fetch(ctx, cred, client)` (replacing the placeholder in `claude.go`):
   ...
   - Success → `Snapshot{Provider:"claude", Windows: windows, UsageKnown: any window satisfies UsageKnown && !Synthetic, FetchedAt: now, Source: usage.SourceOAuth, Confidence:"live"}`.
