...
- Build one invocation-local index of `in.CatalogRows`, keyed by cleaned model name and preserving catalog order within each bucket.
- For each provider in `in.Providers` order:
  ...
  - Pass 2 — derive: `providerErr := error(nil)`; `autoRoutes := []Route{}`; `providerUnrouted := []UnroutedModel{}`. For each `modelID` in `order`:
    - `clean := identity.CleanModelName(seen[modelID].entry.Name)`; `levels, candidates, unmatched := joinModelCandidates(seen[modelID].entry, index[clean])`. Production joins only the matching name bucket; the standalone `joinModel` wrapper retains its test-facing signature.
