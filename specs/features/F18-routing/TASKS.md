...
- `ProduceRoutes` supplies exact-cleaned-name rows when any exist; otherwise it supplies rows from `identity.ModelNameKey(clean)`. `candidates` = every supplied row (order preserved) whose `identity.ModelNameKey(row.Model) == identity.ModelNameKey(clean)`.
- For each level in `declared`, in order: `collapsed` = `identity.CollapseReasoning(level)`; find the first candidate whose collapsed reasoning equals `collapsed`. Found -> append `joinedLevel{level, row}`; not found -> append the level to `unmatched`. Distinct levels may match the same row (e.g. declared `["default","high"]` both collapse to `high`); each yields its own `joinedLevel`.
- The `candidates` return value holds EVERY name-key match in catalog order. `ProduceRoutes` rejects a normalized fallback when those rows contain multiple distinct cleaned model names. When `entry.Reasoning` is empty and exactly one candidate exists but `"default"` does not match its reasoning, append `joinedLevel{candidate.Reasoning, candidate}` and clear `unmatched`: the sole identity is deterministic.
...
- For each provider in `in.Providers` order:
  ...
  - Pass 2 — derive: `providerErr := error(nil)`; `autoRoutes := []Route{}`; `providerUnrouted := []UnroutedModel{}`. For each `modelID` in `order`:
    - `clean := identity.CleanModelName(seen[modelID].entry.Name)`; prefer the exact `catalogByName[clean]` bucket, else use `catalogByNameKey[identity.ModelNameKey(clean)]`; then call `joinModel` with that bucket.
    ...
    - ambiguous if normalized fallback yields multiple distinct cleaned model names, or if `entry.Reasoning` is empty and fewer same-name candidates are covered than matched; record the first `AmbiguityError` for the provider and CONTINUE to the next model (SPEC §2.8). Explicit declared efforts select their matching identities for a unique name; undeclared score efforts are not provider candidates.
    - else: for each `joinedLevel` in `levels`: append `Route{Provider, modelID, level.row.Model, level, BindWindowIDs(provider.Windows, modelID, clean), seen[modelID].src}`. Automatic route model names use the matched canonical score name.
  ...
  - Emit: if `providerErr != nil` -> `result.Errors[provider.Provider] = providerErr`. Always append successful `autoRoutes` then `declaredRoutes` to `result.Routes`, retaining unaffected models under errored providers. Append `providerUnrouted` to `result.Unrouted`.
...
| 11 | provider A has clean `m1` and ambiguous `m2` (its cleaned name matches multiple effort rows with no declared levels), provider B clean | error non-nil and an `*AmbiguityError` for A; `Errors` has A; A's `m1` route and user-declared routes remain; `m2` is absent; B's routes are present |
