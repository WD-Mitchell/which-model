...
6. **Identity join.** Each provider-native model is matched to catalog rows by the F07 identity key: `identity.IdentityKey(cleanName, level)` must equal `identity.IdentityKey(row.Model, row.Reasoning)` — equivalently cleaned-name equality (via `identity.CleanModelName`) plus `identity.CollapseReasoning` equality, so a provider-declared `"default"` level matches a catalog `"high"` row. A non-reasoning model (no declared effort levels) produces a single route with `Reasoning = "default"`; a model with declared levels produces one route per level, in declaration order. `Route.Model` and `Route.Reasoning` carry the provider-side labels; matching applies the collapse. (annex-b §7.1 step 3 and §3.2; master plan §4.2)
...
8. **Ambiguity is a hard error.** If a provider-native model's cleaned name matches catalog identities that its declared effort levels cannot disambiguate (e.g. two catalog rows differ only by a reasoning label the provider does not expose), route production for that provider fails with `*AmbiguityError` naming the provider, the model id, and every matched candidate identity. Auto-derived routes for that provider are NOT produced, and the caller MUST NOT persist them, until an operator resolves the ambiguity with a manual override. User-declared routes for the provider are unaffected. There is no guessing, no "first match", no "best-scoring" choice. (annex-b §7.1 step 4 "Ambiguous" and step 5; master plan §4.2)
...
|---|---|---|
| Ambiguity behaviour | Hard `*AmbiguityError` listing every matched catalog identity; aborts auto-derivation for that provider; user-declared routes unaffected; caller must not persist the provider's auto routes | annex-b §7.1 step 4: "MUST NOT pick the first or best-scoring candidate"; operator resolves via a manual override (step 5) |
...
| Reasoning matching | F07 `IdentityKey` with `CollapseReasoning`; route stores the provider-side label (`"default"` for non-reasoning models) | annex-b §3.2 collapse rule; §7.1 step 3 "a non-reasoning model gets a single Reasoning = default route" |
...
| Ambiguity candidate list | All matched catalog identities named, in catalog order | the operator needs the full set to write a correct override |
