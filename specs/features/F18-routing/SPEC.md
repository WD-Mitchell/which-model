...
6. **Identity join.** Each provider-native model is matched to catalog rows with exact cleaned-name precedence, then the lookup-only `identity.ModelNameKey` fallback (F07 §2.8). The fallback ignores case, whitespace, and non-alphanumeric punctuation. If the fallback key identifies more than one distinct cleaned catalog model name, that model is ambiguous; matching never chooses by row order or score. For a unique model name, `identity.CollapseReasoning` compares declared effort levels to catalog rows, so provider `"default"` matches catalog `"high"`. A model with declared effort levels produces one route per matching declared level, in declaration order; same-name score rows for efforts the provider does not declare are outside that provider-native model's capability and do not make the match ambiguous. A non-reasoning model uses `"default"` when that matches the sole catalog identity, otherwise it adopts the sole identity's reasoning label. `Route.Model` carries the canonical score-catalog name, while `Route.Reasoning` carries the provider-side declared label or that sole inferred label. (annex-b §7.1 step 3 and §3.2; master plan §4.2)

7. **Absent match.** A provider-native model with no exact cleaned-name or normalized-key catalog match is skipped with a warning and an `UnroutedModel` entry — surfaced, never silent, never a hard error. A declared level with no catalog row skips only that level (same warning and entry, with the level named); the model's other levels still route. (annex-b §7.1 step 4 "Absent"; master plan §4.2)

8. **Ambiguity is a hard error, isolated to one model.** An effort-less provider-native model is ambiguous when its exact/normalized name identifies multiple catalog identities that it cannot distinguish. Any normalized-name fallback that maps to multiple distinct catalog model names is also ambiguous, even if the provider declares efforts. `*AmbiguityError` names the provider, model id, and every matched candidate identity, and describes whether the collision is by normalized name or undeclared effort. The ambiguous model produces no auto-derived routes; other unambiguous models from that provider still produce routes. `BuildResult.Errors` and the returned error remain fail-loud, and user-declared routes remain unaffected. Explicit provider effort levels select matching identities for a unique catalog model name; same-name identities at undeclared efforts are ignored. An effort-less sole candidate is deterministic rather than ambiguous. There is no guessing, no first-match choice, and no best-score choice. (annex-b §7.1 step 4 "Ambiguous" and step 5; master plan §4.2)
...
- `ProduceRoutes` returns a non-nil error iff at least one provider hit a hard error; the returned error is the first `*AmbiguityError` in deterministic order (provider input order, then merged model source order). `BuildResult.Errors` maps EVERY provider with a hard error to its first model error, so callers can surface all affected providers rather than only the first.
- On ambiguity, `BuildResult.Routes` contains user-declared routes for all providers plus every successful auto-derived model route, including successful sibling models from a provider that also has an ambiguity. Ambiguous models themselves contribute no automatic routes. The caller persists this partial table before returning the hard error, so unaffected providers and models remain usable.
...
|---|---|---|
| Ambiguity behaviour | Hard `*AmbiguityError` for unresolved effort identities or normalized-name collisions; skip only the ambiguous model, retain successful sibling routes, and report the first provider error plus all provider errors | Provider capability data is authoritative, while a matcher must not select the first or best-scoring candidate; model-local containment preserves unrelated usable routes and the error directs the operator to a manual override |
