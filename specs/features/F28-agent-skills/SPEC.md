...
3. **Repo-root resolution**: install/remove/list resolve the repository root by walking upward from the current directory to the nearest ancestor containing `.git/`; `--repo <path>` overrides. Running outside a repo without `--repo` is exit 1 with a message naming the resolution rule (same upward-walk convention as project-local config/evidence, `docs/plan/annex-d-cli-reference.md §4.6`).
