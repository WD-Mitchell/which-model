 ...
 *  so the sparkline reads as "what this use case cares about" (mockup binding
...
export function UseCasesPage({ detail, openDetail, closeDetail }: PageComponentProps) {
  ...
  const handleNew = useCallback(async () => {
    ...
    while (true) {
      ...
      try {
        ...
        toast.show('new use case created')
  ...
  return (
    <div className={styles.page}>
      <DetailHeader
        title={PAGE_META['Use Cases'][0]}
        blurb={PAGE_META['Use Cases'][1]}
        action={{ label: PAGE_META['Use Cases'][2] as string, onAction: () => void handleNew(), disabled: creating }}
      />
      <span className={'mono ' + styles.kicker}>use cases</span>
      ...
      {profilesList.length === 0 ? (
        <div className={styles.placeholder}>{'No use cases.'}</div>
      ) : (
        profilesList.map((p) => {
            ...
            >
              ...
              <span className={styles.actions} onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  ...
                  title={p.builtin ? 'Built-in use case — cannot be deleted' : `Delete ${p.slug}`}
                  aria-label={p.builtin ? 'Built-in use case — cannot be deleted' : `Delete ${p.slug}`}
      ...
      <p className={styles.note}>
        {'Picks count every launch made with the use case — from the popover and from the '}
  ...
  return (
    <div className={styles.page}>
      ...
      <DetailHeader
        ...
        blurb={
          readOnly
            ? 'A built-in use case — its weights are read-only. Duplicate it to make a version you can change.'
            : 'Drag a weight to change how much this use case cares about each capability. Zero means the capability is ignored.'
        }
        backLabel="Use Cases"
        onBack={onBack}
      />
      {current.description && <p className={styles.note}>{current.description}</p>}
      {current.evidence_note && <p className={styles.note}>{current.evidence_note}</p>}
      <div className={styles.detailStrip}>
        ...
        <span className={'mono ' + styles.summary}>
          {`${weighted} of ${total} priorities weighted · ${current.picks.toLocaleString()} picks`}
        </span>
        <span className={styles.detailActions}>
          ...
          <button
            ...
            title={readOnly ? 'Built-in use case — cannot be deleted' : 'Delete this use case'}
            aria-label={readOnly ? 'Built-in use case — cannot be deleted' : 'Delete this use case'}
      ...
      <div className={styles.sectionHead}>
        <span className={styles.sectionLabel}>core priorities</span>
      ...
      <div className={styles.sectionHead + ' ' + styles.sectionHeadTask}>
        <span className={styles.sectionLabel}>task capabilities</span>
      ...
      {taskRows.length === 0 ? (
        <div className={'mono ' + styles.rowsEmpty}>
          {'no task capabilities weighted — only the core priorities affect this use case'}
        </div>
      ) : (
        <div className={styles.rows}>
          {taskRows.map((r) => (
