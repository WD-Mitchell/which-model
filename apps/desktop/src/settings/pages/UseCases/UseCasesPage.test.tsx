...
it('offers canonical task controls on a profile with no saved task weights', async () => {
 resetHost()
 const { host, profile } = await custom()
 await host.profiles.save({ ...profile, tier2_weights: {} })
 const view = renderPage('custom')
 const planning = await screen.findByRole('slider', { name: 'planning_capability' })
 expect(planning.getAttribute('aria-valuenow')).toBe('0')
