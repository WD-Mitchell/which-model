...
func Render(pc *PublishConfig) ([]byte, error) {
	...
	fmt.Fprintf(&b, "    runs-on:\n")
	fmt.Fprintf(&b, "      group: WDM-Small\n")
	...
	fmt.Fprintf(&b, "      - uses: actions/setup-go@%s # v6.3.0\n", SetupGoPin)
	fmt.Fprintf(&b, "        with:\n")
	fmt.Fprintf(&b, "          go-version-file: go.mod\n")
	fmt.Fprintf(&b, "          cache: true\n")
	fmt.Fprintf(&b, "      - run: go build -o which-model ./cmd/which-model\n")
	if pc.RunTests {
		fmt.Fprintf(&b, "      - run: go test ./internal/catalog/... ./internal/pick/... ./internal/routing/...\n")
	}
	fmt.Fprintf(&b, "      - run: ./which-model catalog refresh\n")
	fmt.Fprintf(&b, "        env:\n")
	fmt.Fprintf(&b, "          ARTIFICIAL_ANALYSIS_API: ${{ secrets.ARTIFICIAL_ANALYSIS_API }}\n")
	fmt.Fprintf(&b, "      - id: changes\n")
	fmt.Fprintf(&b, "        run: |\n")
	fmt.Fprintf(&b, "          git add -- %s %s\n", pc.RawCSVPath, pc.ScoresCSVPath)
