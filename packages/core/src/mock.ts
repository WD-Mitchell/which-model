...
export function createMockEngineHost(
  ...
  const activeSignInFlows = new Map<string, string>()
  ...
  const host: EngineHost & { data: MockData } = {
    ...
    catalog: {
      ...
      async models(): Promise<CatalogModel[]> {
        ...
        for (const m of data.models) {
          ...
          for (const provider of mProviders) acc.providers.add(provider)
          if (rank >= acc.topRank) {
            acc.topRank = rank
            acc.id = m.id
        ...
        for (const p of data.providers) {
          ...
          for (const pm of providerModels(p.id)) {
            ...
            if (!acc) {
              acc = { name: pm.model_name, id: pm.model_id, reasoning: [], intel: null, cost: null, speed: null, providers: new Set<string>(), topRank: -1 }
    ...
    signin: {
      ...
      async saveAPIKey(provider, accountName, apiKey) {
        ...
        const accounts = mockAccounts[provider] ?? []
        const existing = accounts.findIndex((account) => account.name === accountName)
        const next = { name: accountName, kind: 'token' as const, ref: 'which-model' }
