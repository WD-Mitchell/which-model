...
export function RankList({ items, index, onPick }) {
    return (_jsx("div", { className: styles.band, children: items.map((model, i) => {
            ...
            return (_jsxs("button", { type: "button", "aria-current": selected ? 'true' : undefined, className: cx(styles.row, selected && styles.rowSelected), onClick: () => onPick(i), children: [_jsx("span", { className: styles.rank, children: model.rank }), _jsxs("span", { className: styles.identity, children: [_jsx("span", { className: cx(styles.name, selected && styles.nameSelected), children: name }), _jsx(MissingBenchmarkNotice, { model: model })] }), _jsx("span", { className: cx(styles.score, selected && styles.scoreSelected), children: model.score.toFixed(2) }), _jsx("span", { className: styles.route, children: provider })] }, model.route_key));
        }) }));
}
