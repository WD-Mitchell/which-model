...
- Benchmark `version` metadata is ignored entirely — never validated, never used to pick a winner.  The original `_version()` shape validation (`v?\d+(\.\d+)*` / `"Month YYYY"`) was removed 2026-09-01 after models.dev started emitting free-form versions (`"1.1 Main"`) that broke the collector; the max-score policy never consumed the value.
