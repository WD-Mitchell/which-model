...
Verify the release evidence using [the release verification instructions](security/release-verification.md), then extract the selected archive and copy the app to Applications. Release bundles must pass [Developer ID signing, Apple notarization and Gatekeeper assessment](releases/macos-signing.md). Company distribution still follows the organization's macOS software approval process. GitHub attestation authenticates release evidence separately from Apple notarization. Earlier ad-hoc preview candidates do not satisfy the current release requirement.
